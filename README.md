...
**Background Script**:
```
chrome://extensions/ → EduVip → "service worker" link
```

**Content Script**:
```
F12 DevTools trên target page → Console tab
```
...
- ✅ Use AbortController cho cleanup event listeners
- ✅ Clear timers và disconnect observers on unload
- ✅ Log errors với context cho debugging
- ❌ Không hardcode sensitive data
- ❌ Không block main thread với heavy operations
...
- ✅ Memory leak fixes (event listeners, timers, observers)
- ✅ Input validation và sanitization

**Priority 2 - High**:
- ✅ Loading states cho tất cả buttons
- ✅ User-friendly error messages (tiếng Việt)
- ✅ Security warning về API key storage

**Bug Fixes**:
- 🐛 Fixed AI comment showing empty content (removed over-aggressive sanitization)
...
> ⚠️ API key được encrypt với XOR-based obfuscation. Đây là basic protection, không cryptographically secure. Không nên sử dụng cho production với high-security requirements.

**Recommendations**:
- Sử dụng API key với limited quota
- Không share extension với người khác
- Consider backend proxy cho production

### Content Script Conflicts

Extension có thể conflict với other extensions cũng modify DOM. Nếu gặp issues:
- Disable other extensions temporarily
- Check console cho error messages
- Report bugs với logs
...
**Priority 3 - Medium**:
- [ ] Consolidate constants vào shared module
...
- [ ] Configuration UI cho site URLs
